...
public class StaticAbstractGenerator : IIncrementalGenerator {
    ...
    private static void Generate(
        ...
        var supportsDefaultInterfaceMethods = supportsVirtualStatics || compilation.GetTypeByMetadataName("System.Runtime.CompilerServices.RuntimeFeature")
            ?.GetMembers("DefaultImplementationsOfInterfaces").Any() == true;
        var isCSharp8OrGreater = (compilation as CSharpCompilation)?.LanguageVersion >= LanguageVersion.CSharp8;
        var supportsInterfaceForwards = supportsDefaultInterfaceMethods && isCSharp8OrGreater;
    ...
    private static Dictionary<INamedTypeSymbol, List<ITypeSymbol>> CollectConformingExternalTypes(
        ...
        foreach (var reg in allRegistrations) {
            if (reg.TargetInterface != null) {
                AddConformingTypes(result, reg.TargetInterface.OriginalDefinition, reg.Types, interfaceInfosMap);
            }
        }
    ...
    private static void AddConformingTypes(
        ...
        targetInterface = targetInterface.OriginalDefinition;
    ...
    private static (INamedTypeSymbol? TargetInterface, List<ITypeSymbol> Types, bool Strict) ParseStaticRegisterAttribute(
                ...
                foreach (var arg in syntax.ArgumentList.Arguments) {
                    if (arg.NameEquals != null) {
                        ...
                        if (name == "Strict")
                            ...
                            targetInterface = (semanticModel.GetTypeInfo(toe.Type).Type as INamedTypeSymbol)?.OriginalDefinition ?? targetInterface;
                    }
                }
            }
        ...
        return (targetInterface?.OriginalDefinition, types, strict);
    }
