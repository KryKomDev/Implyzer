...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(IndirectImplCodeFixProvider))]
[Shared]
public class IndirectImplCodeFixProvider : CodeFixProvider {
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context) {
        foreach (var diagnostic in context.Diagnostics) {
            ...
            if (node is not BaseTypeSyntax baseTypeNode)
                continue;
            ...
            if (!string.IsNullOrEmpty(suggestion))
                context.RegisterCodeFix(
                    CodeAction.Create(
                        $"Implement '{suggestion}' instead",
                        c => ReplaceInterfaceAsync(context.Document, baseTypeNode, suggestion!, c),
                        "ReplaceInterface"
                    ),
                    diagnostic
                );
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    ...
                    c => RemoveInterfaceAsync(context.Document, baseTypeNode, c),
                    "RemoveInterface"
                ),
                diagnostic
            );
