...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(ImplTypeInheritanceCodeFixProvider))]
[Shared]
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context) {
        foreach (var diagnostic in context.Diagnostics) {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title,
                    c => AddBaseTypeAsync(context.Document, typeDecl, requiredBaseType, c),
                    "InheritBaseType"
                ),
                diagnostic
            );
